...
export function loadOnServer({ components, params }, store, helpers) {
  return Promise.all(
    ...
    .catch(error =>
      console.error("reduxAsyncConnect server promise error: ", error)
    )
    .then(() => {
...
class ReduxAsyncConnect extends React.Component {
  ...
  loadAsyncData(props) {
    ...
    const promises = asyncConnectPromises(
      filterAndFlattenComponents(components),
      params,
      store,
      helpers
...
export default connect(null, { beginGlobalLoad, endGlobalLoad })(
  ReduxAsyncConnect
);
